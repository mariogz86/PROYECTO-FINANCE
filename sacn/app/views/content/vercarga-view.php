...
function cargardatos(alerta) {
    ...
    if (array.esdetalle == 1) {
        if (array.datos != "[]") {
            cargargriddetalles(JSON.parse(array.datos));
        }
    } else {
        if (array.datos != "[]") {
...
function cargargriddetalles(datos) {

 
  // Lista de claves que quieres ocultar
const columnasOcultas = ["CASO", "CIIU", "ANUAL", "archivo", "id_formulario", "id_archivofuente","INDICE","existe","CIIU_R4","TIPO","LINEA","TIPCUEST"];

// Obtener claves filtradas desde el primer objeto
const allKeys = Object.keys(datos[0]);
const keysFiltradas = allKeys.filter(key => !columnasOcultas.includes(key));

// Crear definición de columnas para DataTables
const columns = keysFiltradas.map(key => ({
  title: key,
  data: key
}));
...
if ($.fn.DataTable.isDataTable('#myTable')) {
  ...
  // 2. Limpiar el contenido del <table> (cabezera y cuerpo)
  $('#myTable').empty();

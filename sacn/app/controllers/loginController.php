...
//declaracino de funciones 
	class loginController extends mainModel{
				...
				echo '<article class="message is-danger">
				  <div class="message-body">
				    ...
				    No has llenado todos los campos que son obligatorios
						...
						FROM \"SACNSYS\".usuarios u  
						inner join \"SACNSYS\".roles r on r.id_rol=u.id_rol
					    ...
					    if($check_usuario->rowCount()==1){
							...
							$_SESSION['foto']="default"; 
							 ...
							 $ubloqueado=$check_usuario['u_bloqueado'];
							...
							//se valida si ya tiene 3 intentos o el estado del usuario esta de baja
							 if (($cantidadintentos==3 || $_SESSION['estado']==0) ||($ubloqueado==1)) {
					    	...
					    	if($check_usuario['usuario']==$usuario && password_verify($clave,$check_usuario['u_clave'])){
								...
								$vistas=$this->ejecutarConsulta("SELECT o.nombrevista FROM \"SACNSYS\".rol_opcion RO 
								inner join \"SACNSYS\".opcion o on o.id_opcion=ro.id_opcion 
								...
								if($hoy>=$_SESSION['fechavenc'] || $check_usuario['reset_clave'] ==1 ){
									$_SESSION['cambioclave']="1";
										...
										if($check_usuario['reset_clave'] ==  1){
											...
											Se solicito reseteo de contraseña o es usuario nuevo, proceda a cambiar clave.
											</div>
										</article>';
										}else{
											echo '<article class="message is-warning">
		...
		public function cambiarclave(){
			...
			if(password_verify($claveant,$_SESSION['claveant'])){
				...
				$clave=password_hash($clavenue,PASSWORD_BCRYPT,["cost"=>10]);
					...
					$sentencia ="select \"SACNSYS\".cambiar_clave(0,'".$clave."','".$date."','".$_SESSION['id']."');";
		...
		public function actualizarintentos($cantidad,$idusuario){
			...
			if ($cantidad==3){				
				$sentencia ="select \"SACNSYS\".actualizar_intentos('".$cantidad."','1','".$_SESSION['id']."');";
				$sql=$this->actualizarDatos($sentencia);
			$sql->execute();

			$this->enviarcorreocuentabloqueada();
			}else{
				$sentencia ="select \"SACNSYS\".actualizar_intentos('".$cantidad."','0','".$_SESSION['id']."');";
				$sql=$this->actualizarDatos($sentencia);
			$sql->execute();
			}
		}
		...
		public function actualizarclave($clave,$idusuario){
			...
			$sentencia ="select \"SACNSYS\".cambiar_clave(1,'".$clavenueva."','".$hoy."','".$idusuario."');";
			$sql=$this->actualizarDatos($sentencia);
			$sql->execute();

<?php
	...
	class menuController extends mainModel{
		...
		public function guardar(){
			...
			if ($_POST["idmenu"]=="0"){
				$sentencia ="select \"SACNSYS\".insertar_menu('".$nombre."','".$icono."','".$orden."','0','".$_SESSION['id']."');";
				$sql=$this->actualizarDatos($sentencia);
				$sql->execute();
			}else{				
				$sentencia ="select \"SACNSYS\".actualizar_menu('".$_POST["idmenu"]."','".$nombre."','".$icono."','".$orden."','0','".$_SESSION['id']."');";
				$sql=$this->actualizarDatos($sentencia);
				$sql->execute();	
        ...
        public function Buscarmenu(){
            ...
            if ($_POST["idmenu"]=="0"){
                $sentencia ="select * from \"SACNSYS\".OBTENER_MENU where UPPER(nombre)=UPPER('".$nombre."');  ";
                $datos = $this->ejecutarConsulta($sentencia);
                $datos = $datos->fetchAll();
            }else{				
                $sentencia ="select * from \"SACNSYS\".OBTENER_MENU where UPPER(nombre)=UPPER('".$nombre."') and id_menu not in ('".$_POST["idmenu"]."');  ";
                $datos = $this->ejecutarConsulta($sentencia);
			$datos = $datos->fetchAll();
        ...
        public function validar_inactivar(){
            ...
            $sentencia ="select * from \"SACNSYS\".OBTENER_ROLMENU where id_menu='".$_POST["id_menu"]."' and estadorol=1;  ";
            $datos = $this->ejecutarConsulta($sentencia);
			$datos = $datos->fetchAll();
		...
		public function cambiarestado($estado){ 
			...
			$idcat = $_POST["id_menu"];
				$sentencia ="select \"SACNSYS\".CAMBIARESTADO_MENU('".$idcat."','".$estado."','".$_SESSION['id']."');";
				$sql=$this->actualizarDatos($sentencia);
				$sql->execute(); 

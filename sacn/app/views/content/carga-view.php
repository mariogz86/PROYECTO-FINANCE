...
$("#titulo")[0].innerText = "Esta pantalla permite guardar en la base de datos la información de los archivos .SAV asociados al formulario";
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                var dato = JSON.stringify(tabla.data().filter((datos) => datos.existe == "NO")
                    .toArray()).replaceAll("&", "%26").replaceAll("%", "porc.");
                $(".loadersacn")[0].style.display = "";
                $.ajax({
                    type: "POST",
                    url: "<?php  echo APP_URL.'ajax/cargaAjax.php' ?>",
                    data: "Cargaguardar=guardar&formulario="+selectVal+"&datos="+ decodeURIComponent(dato) + "&nuevas=1",
...
//                         type: "POST",
//                         url: "<?php  echo APP_URL.'ajax/cargaAjax.php' ?>",
//                         data: "modulo_Opcion=carga&cmb_formulario="+selectform+"&cmb_archivofuente="+selectarchivo,
//                         success: function(response) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    var dato = JSON.stringify(tabla.rows({
                        selected: true
                    }).data().toArray()).replaceAll("&", "%26").replaceAll("%", "porc.");
                    $(".loadersacn")[0].style.display = "";
                    $.ajax({
                        type: "POST",
                        url: "<?php  echo APP_URL.'ajax/cargaAjax.php' ?>",
                        data: "Cargaguardar=guardar&formulario="+selectVal+"&datos=" + decodeURIComponent(dato) + "&nuevas=0",
                        success: function(response) {

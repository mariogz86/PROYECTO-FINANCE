<?php
	...
	class valcatalogoController extends mainModel{
        ...
        public function listarvalcatalogo(){
			$consulta_datos="select * from \"SACNSYS\".obtener_valcatalogo;"; 
		...
		public function guardar(){
			...
			if ($_POST["idcatalogovalor"]=="0"){
				$sentencia ="select \"SACNSYS\".insertar_valorcat('".$idcat."','".$nombre."','".$descripcion."','".$_SESSION['id']."');";
				$sql=$this->actualizarDatos($sentencia);
				$sql->execute();
			}else{
				$sentencia ="select \"SACNSYS\".actualizar_valorcat('".$_POST["idcatalogovalor"]."','".$idcat."','".$nombre."','".$descripcion."','".$_SESSION['id']."');";
				$sql=$this->actualizarDatos($sentencia);
				$sql->execute();	
		...
		public function cambiarestado($estado){ 
			...
			$idcat = $_POST["id_catalogovalor"];
				$sentencia ="select \"SACNSYS\".cambiarestado_valcatalogo('".$idcat."','".$estado."','".$_SESSION['id']."');";
				$sql=$this->actualizarDatos($sentencia);
				$sql->execute(); 

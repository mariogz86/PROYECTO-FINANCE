...
class usuarioController extends mainModel
{
	...
	public function listarusuarios()
	{
		...
		$consulta_datos = "select * from \"SACNSYS\".OBTENER_USUARIOS;";
	...
	public function Buscarusuario()
	{
		if ($_POST["idusuario"] == "0") {
			$consulta_datos = "select count(*) from \"SACNSYS\".OBTENER_USUARIOS where usuario='" . $_POST["usuario_usuario"] . "';";
		} else {
			$consulta_datos = "select count(*) from \"SACNSYS\".OBTENER_USUARIOS where usuario='" . $_POST["usuario_usuario"] . "' and id_usuario not in ('" . $_POST["idusuario"] . "');";
		}
	...
	public function guardar()
	{
		...
		if ($_POST["idusuario"] == "0") {
			$sentencia = "select \"SACNSYS\".INSERTAR_USUARIO('" . $usuario_nombre . "','" . $usuario_apellido . "','" . $usuario_usuario . "','" . $usuario_email . "','" . $clavenueva . "','" . $cmb_rol . "','" . $fechavencimiento . "','" . $_SESSION['id'] . "');";
			...
			$sentencia = "select \"SACNSYS\".ACTUALIZAR_USUARIO('" . $_POST["idusuario"] . "','" . $usuario_nombre . "','" . $usuario_apellido . "','" . $usuario_usuario . "','" . $usuario_email . "','" . $clave . "','" . $cmb_rol . "','" . $fechavencimiento . "','" . $_SESSION['id'] . "');";
			$sql = $this->actualizarDatos($sentencia);
			$sql->execute();
			$total = (int) $sql->fetchColumn();
		}
	...
	public function bloquear($estado)
	{
		...
		$sentencia = "select \"SACNSYS\".BLOQUEAR_USUARIO('" . $idcat . "','" . $estado . "','" . $_SESSION['id'] . "');";
		$sql = $this->actualizarDatos($sentencia);
		$sql->execute();
	...
	public function cambiarestado($estado)
	{
		...
		$idcat = $_POST["id_usuario"];
		$sentencia = "select \"SACNSYS\".cambiarestado_usuario('" . $idcat . "','" . $estado . "','" . $_SESSION['id'] . "');";
		$sql = $this->actualizarDatos($sentencia);
		$sql->execute();
	...
	public function usuariocreado($nombre, $apellido, $usuario, $clave)
	{
		...
		$phpmailer->Body    = "Estimado(a) <b>" . $nombre . " " . $apellido . "</b>:<br><br>"
			. "Le informamos que su cuenta de usuario <b>" . $usuario . "</b> del sistema <b>Automatizado de Cuentas Nacionales SACN</b> ha sido creada, "

...
CREATE OR REPLACE VIEW "SYSTEM".reportejobs AS
SELECT
	T.ID_TRABAJO,
	T.NUM_REFERENCIA,
	T.U_ESTADO,
	...
	C.NOMBRE AS COMPANY,
	CLI.FULL_NAME,
	COALESCE(CVP.NOMBRE, 'undefined') AS PAY,
	CONCAT(
		COALESCE(U.U_NOMBRE_COMPLETO, 'Not Assigned'),
		' ',
		U.U_APELLIDO_COMPLETO
	) AS TECNICO,
	COALESCE(PAGOS.TOTALSERVICE, 0) as TOTALSERVICE,
	COALESCE(PAGOS.TOTALLABOR, 0) as TOTALLABOR ,
	COALESCE(PAGOS.TOTALPARTE, 0) as TOTALPARTE,
	COALESCE(PAGOS.GRANTOTAL, 0) AS TOTALFACTURA,
	T.FECHA_CREACION,
	T.ID_TECNICO
FROM
	"SYSTEM".TRABAJO T
	JOIN "SYSTEM".catalogovalor cve ON cve.id_catalogovalor = t.id_estadotrabajo
	INNER JOIN "SYSTEM".COMPANY C ON C.ID_COMPANY = T.ID_COMPANY
	INNER JOIN "SYSTEM".CLIENTE CLI ON CLI.ID_CLIENTE = T.ID_CLIENTE
	LEFT JOIN (
		SELECT
			S.ID_TRABAJO,
			SUM(S.SERVICEFEE) AS TOTALSERVICE,
			SUM(S.COVERED) AS TOTALCOVERED,
			SUM(L.TOTALLABOR) AS TOTALLABOR,
			SUM(TP.TOTALPARTE) AS TOTALPARTE,
			SUM(S.SERVICEFEE + L.TOTALLABOR + TP.TOTALPARTE) AS GRANTOTAL
		FROM
			"SYSTEM".SERVICIO S
			INNER JOIN (
				SELECT
					ID_SERVICIO,
					SUM(CANTIDAD * COSTO) AS TOTALPARTE
				FROM
					"SYSTEM".PARTE
				GROUP BY
					ID_SERVICIO
			) AS TP ON TP.ID_SERVICIO = S.ID_SERVICIO
			INNER JOIN (
				SELECT
					ID_SERVICIO,
					SUM(LABORFEE) AS TOTALLABOR
				FROM
					"SYSTEM".DIAGNOSTICO
				GROUP BY
					ID_SERVICIO
			) AS L ON L.ID_SERVICIO = S.ID_SERVICIO
		GROUP BY
			S.ID_TRABAJO
	) AS PAGOS ON PAGOS.ID_TRABAJO = T.ID_TRABAJO
	LEFT JOIN "SYSTEM".PAYMENT P ON P.ID_TRABAJO = T.ID_TRABAJO
	LEFT JOIN "SYSTEM".CATALOGOVALOR CVP ON CVP.ID_CATALOGOVALOR = P.ID_VALPAYMENT
...
CREATE OR REPLACE VIEW "SYSTEM".OBTENER_REPORTESERVICE AS
SELECT
	U.ID_REPORTESERVICIO,
	U.ID_TRABAJO,
	U.ID_VALAPPLIANCE,
	U.ID_VALBRAND,
	U.ID_VALORTIPOCABLE,
	U.ID_VALORFACTORFALLA,
	U.MODEL,
	U.SERIAL,
	U.PROBLEMDETAIL,
	U.LABORCOST,
	...
	CVA.NOMBRE AS APPLIANCE,
	CVB.NOMBRE AS BRAND,
	CVS.NOMBRE AS TIPOCABLE,
	CVE.NOMBRE AS FALLA,
	UM.USUARIO AS USUARIOM,
	UU.USUARIO AS USUARIO
FROM
	"SYSTEM".REPORTESERVICIO U
	LEFT JOIN "SYSTEM".CATALOGOVALOR CVA ON CVA.ID_CATALOGOVALOR = U.ID_VALAPPLIANCE
	LEFT JOIN "SYSTEM".CATALOGOVALOR CVB ON CVB.ID_CATALOGOVALOR = U.ID_VALBRAND
	LEFT JOIN "SYSTEM".CATALOGOVALOR CVS ON CVS.ID_CATALOGOVALOR = U.ID_VALORTIPOCABLE
	LEFT JOIN "SYSTEM".CATALOGOVALOR CVE ON CVE.ID_CATALOGOVALOR = U.ID_VALORFACTORFALLA
	JOIN "SYSTEM".USUARIOS UU ON UU.ID_USUARIO = U.USUARIO_CREACION
	LEFT JOIN "SYSTEM".USUARIOS UM ON UM.ID_USUARIO = U.USUARIO_MODIFICA
ORDER BY
	U.ID_REPORTESERVICIO ASC;
	
	
	
	CREATE
OR REPLACE FUNCTION "SYSTEM".GUARDARREPORTESERVICIO (
	ID_TRABAJO INT,
	ID_VALAPPLIANCE INT,
	ID_VALBRAND INT,
	MODEL VARCHAR(250),
	SERIAL VARCHAR(250),
	PROBLEMDETAIL VARCHAR(2500),
	ID_VALORTIPOCABLE INT,
	OTROTIPOCABLE VARCHAR(500),
	ID_VALORFACTORFALLA INT,
	OTROFACTORFALLA VARCHAR(500),
	LABORCOST DOUBLE PRECISION,
	REQUIEREPARTES SMALLINT,
	COMPLETOREPARACION SMALLINT,
	DATOPARTES JSONB,
	IDUSUARIO INT
) RETURNS SETOF INT AS $$   
begin  
...
VALUES (ID_TRABAJO,
		...
		REQUIEREPARTES,
		COMPLETOREPARACION,
		DATOPARTES,CURRENT_DATE,IDUSUARIO )
RETURNING
	ID_REPORTESERVICIO;
...
CREATE
OR REPLACE FUNCTION "SYSTEM".ACTUALIZARREPORTESERVICIO (
	REPT_ID_REPORTESERVICIO INT,
	REPT_ID_TRABAJO INT,
	REPT_ID_VALAPPLIANCE INT,
	REPT_ID_VALBRAND INT,
	REPT_MODEL VARCHAR(250),
	REPT_SERIAL VARCHAR(250),
	REPT_PROBLEMDETAIL VARCHAR(2500),
	REPT_ID_VALORTIPOCABLE INT,
	REPT_OTROTIPOCABLE VARCHAR(500),
	REPT_ID_VALORFACTORFALLA INT,
	REPT_OTROFACTORFALLA VARCHAR(500),
	REPT_LABORCOST DOUBLE PRECISION,
	REPT_REQUIEREPARTES SMALLINT,
	REPT_COMPLETOREPARACION SMALLINT,
	REPT_DATOPARTES JSONB,
	REPT_IDUSUARIO INT
) RETURNS SETOF INT AS $$   
begin  

RETURN QUERY
UPDATE "SYSTEM".REPORTESERVICIO
SET
	ID_TRABAJO = REPT_ID_TRABAJO ,
	ID_VALAPPLIANCE = REPT_ID_VALAPPLIANCE,
	ID_VALBRAND = REPT_ID_VALBRAND,
	MODEL =REPT_MODEL ,
	SERIAL = REPT_SERIAL,
	PROBLEMDETAIL =REPT_PROBLEMDETAIL ,
	ID_VALORTIPOCABLE =REPT_ID_VALORTIPOCABLE ,
	OTROTIPOCABLE = REPT_OTROTIPOCABLE ,
	ID_VALORFACTORFALLA = REPT_ID_VALORFACTORFALLA,
	OTROFACTORFALLA =REPT_OTROFACTORFALLA ,
	LABORCOST = REPT_LABORCOST,
	REQUIEREPARTES = REPT_REQUIEREPARTES,
	COMPLETOREPARACION = REPT_COMPLETOREPARACION,
	DATOPARTES =REPT_DATOPARTES, 
	FECHA_MODIFICA =CURRENT_DATE ,
	USUARIO_MODIFICA = REPT_IDUSUARIO
WHERE ID_REPORTESERVICIO=REPT_ID_REPORTESERVICIO
RETURNING
	REPT_ID_REPORTESERVICIO;
...
CREATE
OR REPLACE FUNCTION "SYSTEM".ELIMINARREPORTESERVICIO (IDSER INTEGER) RETURNS SETOF INT AS $$
begin
RETURN QUERY
DELETE FROM  "SYSTEM".reporteservicio
WHERE
	id_reporteservicio  = IDSER
RETURNING
	id_reporteservicio;
end;
$$ LANGUAGE PLPGSQL;
...
CREATE SEQUENCE "SYSTEM".REFERENCIAINVOICE START
WITH
	1370 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1;

CREATE
OR REPLACE FUNCTION "SYSTEM".GENERAR_NUMERO_REFERENCIAINVOICE () RETURNS TEXT AS $$
DECLARE
  sec BIGINT;
BEGIN
  sec := nextval('"SYSTEM".referenciainvoice');
  RETURN 'INV-' || LPAD(sec::TEXT, 7, '0');
END;
$$ LANGUAGE PLPGSQL;

CREATE
OR REPLACE FUNCTION "SYSTEM".INSERTAR_JOB (DATOSJSON JSONB, IDUSUARIO INT) RETURNS SETOF INT AS $$
DECLARE	idcliente INT; 
DECLARE	idestadojob INT; 
...
VALUES (idtrabajo,idestadojob,'job booked',CURRENT_DATE,IDUSUARIO );
 ...
 RETURN QUERY
 UPDATE "SYSTEM".TRABAJO set NUM_REFERENCIA="SYSTEM".generar_numero_referenciainvoice()
...
CREATE OR REPLACE FUNCTION "SYSTEM".ELIMINAR_TRABAJO (IDTRABAJO INTEGER) RETURNS SETOF INT AS $$
begin 
delete from "SYSTEM".movimientotrabajo where id_trabajo=IDTRABAJO;

delete from "SYSTEM".cita where id_trabajo=IDTRABAJO;

delete from "SYSTEM".jobimagenes where trabajo_id =IDTRABAJO;

delete from "SYSTEM".diagnostico where id_servicio
in (select id_servicio from "SYSTEM".servicio where id_trabajo=IDTRABAJO);

delete from "SYSTEM".parte where id_servicio
in (select id_servicio from "SYSTEM".servicio where id_trabajo=IDTRABAJO);

delete from "SYSTEM".servicio where id_trabajo=IDTRABAJO;

delete from "SYSTEM".payment where id_trabajo=IDTRABAJO;
RETURN QUERY
delete from "SYSTEM".trabajo where id_trabajo=IDTRABAJO
RETURNING
	IDTRABAJO;
end;
$$ LANGUAGE PLPGSQL;

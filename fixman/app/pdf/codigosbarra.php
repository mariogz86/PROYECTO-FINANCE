<?php
	...
	if($datos_venta->rowCount()>0){
		...
		$pdf->Cell(0,0,iconv("UTF-8", "ISO-8859-1","Codigos de barra"),0,0,'C');
		...
		$pdf->Cell(185,8,iconv("UTF-8", "ISO-8859-1",'Codigos de Barra'),1,0,'C',true); 
		...
		foreach($venta_detalle as $detalle){
			...
			$code=$detalle['producto_codigo']; 
			$pdf->Cell(185,5,iconv("UTF-8", "ISO-8859-1",$code ),'',1,'C');
			$pdf->Code128(70,$altofila1,$code,80,20);
			$pdf->SetXY(70,$altofila2);
			

			$pdf->Ln(7);  

			$altofila1=$altofila1+30;
			$altofila2=$altofila2+30;

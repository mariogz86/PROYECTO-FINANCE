...
if (isset($_POST['generarfactura'])) {
    ...
    $disclaimer = $insformulario->obtenerdisclaimer("discminvoice");
       ...
       public function Footer()
        {
            ...
            // Establecer color y grosor de la línea
            $this->SetDrawColor(0, 0, 0);
            $this->SetLineWidth(0.2);

            // Dibujar línea de borde a borde (página completa)
            $this->Line(
                15,                          // X inicial: borde izquierdo absoluto
                $this->GetY(),             // Y inicial: posición actual
                200,     // X final: borde derecho absoluto
                $this->GetY()              // Y final: misma línea horizontal
            );

            // Texto de pie de página alineado a la derecha
            $this->SetFont('helvetica', '', 8);
            $this->SetY(-10); // Subir para dejar espacio entre línea y texto
            $this->Cell(0, 10, $this->getAliasNumPage() . ' / ' . $this->getAliasNbPages(), 0, 0, 'R');
        }

        // Colored table
        public function ColoredTable($datos, $servicios, $diagnosticos, $partes,$disclaimer)
        {
          ...
          // Calcular la posición Y cerca del pie de página
        if ($this->GetY() > 230) { // si estás muy cerca del final
                $this->AddPage();
            }
            $this->SetY(-45); 
        // Limpiar y preparar el contenido
        $descripcion = rtrim($disclaimer[0]['descripcion']);
        $texto = nl2br($descripcion);
        ...
        // Escribir la celda en la parte baja
        $this->writeHTMLCell(
            185,           // ancho
            0,             // altura automática
            '',            // x (posición automática)
            '',            // y (ya la definiste con SetY)
            '' . $texto,
            1,             // borde
            0,             // no salto de línea
            true,          // relleno
    ...
    $pdf->Cell(0, 10, $fecha, 0, 1, '');
    // print colored table
    $pdf->ColoredTable($trabajo, $servicios,  $diagnosticos, $partes,$disclaimer);
...
if (isset($_POST['generarreporte'])) {
    $idtrabajo = $_POST['generarreporte'];
    $trabajo = $insformulario->obtenerjobporid($idtrabajo);
    $servicios = $insformulario->obtenerserviciosporidtrabajo($idtrabajo);
    $diagnosticos = $insformulario->obtenerdiagnosticos($idtrabajo);
    $partes = $insformulario->obtenerpartes($idtrabajo);
    $disclaimer = $insformulario->obtenerdisclaimer("discmreport");

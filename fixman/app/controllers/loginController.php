				...
				if ($cantidadintentos == 3 || $_SESSION['estado'] == 0) {
					...
					if ($check_usuario['usuario'] == $usuario && password_verify($clave, $check_usuario['u_clave'])) {
						...
						if ($hoy >= $_SESSION['fechavenc'] || $check_usuario['reset_clave'] == 1) {
							...
							session_write_close(); // asegura que la sesión se cierre antes de redirigir
							$url = APP_URL . "index.php?views=dashboard";

							if (!headers_sent()) {
								header("Location: $url", true, 303); // 303 es más compatible tras un POST
								exit();
							} else {

...
$funciones = new FuncionesController();
    ...
    if(isset($_POST['modulo_Opcion']))
	{
        ...
        if(!empty($archivosnoencontrados)){
            ...
            echo json_encode( $alerta); 
                ...
                for ($colum = 0; $colum <= $totalregistro-1; ++$colum) {
                    ...
                    $consulta_datos="select distinct boleta from \"SACNSYS\".VALORVARIABLE where id_formulario=".$cmb_formulario." and id_archivofuente=".$formulario[$colum]['id_archivofuente'].""; 

                    $consulta_datos_equivalencias="select * from \"SACNSYS\".obtener_cargaequivalencias where id_formulario=".$cmb_formulario." and id_archivofuente=".$formulario[$colum]['id_archivofuente'].""; 
                    ...
                    $datosequivalencias = $funciones->ejecutarconsultaarreglo($consulta_datos_equivalencias); 
                    $buscarvarequivalencia = array_column($datosequivalencias, 'codinide');
                            ...
                            $variableequivalencia="";
                             for ($col = 0; $col <= $highestColumn-1; ++$col) {
                                $datovar = $reader->variables[$col];
                                   ...
                                   // $variables[$datovar->name] =trim(str_ireplace("\"", "", str_ireplace("'", "", str_ireplace(["\r", "\n"], "", $value[$col]))));
                                      // Limpieza más exhaustiva de los saltos de línea y caracteres invisibles
                                      $nomvariable = $datovar->name;
                                       
                                      $value[$col] = str_replace("Ã\x91", "Ñ", $value[$col]);
                                      $clean_value =    mb_convert_encoding($value[$col], 'UTF-8', 'ISO-8859-1');                                      
                                      $clean_value = str_ireplace(["\"", "'","&", "\r", "\n", "\t"], "", $clean_value);  // Elimina saltos de línea y tabulaciones
                                      $clean_value = preg_replace('/\s+/', ' ', $clean_value);  // Elimina múltiples espacios consecutivos
                                      $clean_value = trim(str_ireplace(["\"", "'",], "", $clean_value)); // Elimina comillas y espacios extra
                                     // $variables[$nomvariable] =$clean_value;
                                       if($nomvariable!=$variableequivalencia){
                                                $variables[$nomvariable] =$clean_value;
                                            }
                                      
                                      $found_keyequivalencia = array_search($datovar->name, $buscarvarequivalencia);
                                            if(is_numeric($found_keyequivalencia)){ 

                                             
                                            // Búsqueda
                                            foreach ($datosequivalencias as $registro) {
                                                if (
                                                    //isset($registro['codinide'], $registro['valorinide']) &&
                                                    $registro['codinide'] === $datovar->name &&
                                                    $registro['valorinide'] === $value[$col]
                                                ) {
                                                //    $resultado = $registro;
                                                $variables[$nomvariable] =$clean_value;

                                                $variables[$registro['codnacional']]= $registro['valorcodnacional'] ;

                                                $variableequivalencia=$registro['codnacional'];
        ...
        if(isset($_POST['Cargaguardar'])=="guardar")
        {
            ...
            $result =$inscarga->guardar($archivos,$nuevas,$_POST['formulario']);
